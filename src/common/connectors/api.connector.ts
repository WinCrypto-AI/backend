...
export const wmsApiConnector = createHttpClient({
  baseURL: '',
  timeout: 2 * 60 * 1000,
  beforeRequest: config => {
...
export const apeApiConnector = createHttpClient({
  baseURL: CONNECTOR_APE_WMS_BASE_URL.trim(),
  timeout: 2 * 60 * 1000,
  beforeRequest: config => {
    config.headers['Content-Type'] = EMediaType.APPLICATION_JSON;
    config.headers['Accept'] = EMediaType.APPLICATION_JSON;
    config.headers['x-apikey'] = CONNECTOR_APE_WMS_APIKEY.trim();
    return config;
  },
  handleError: err => {
